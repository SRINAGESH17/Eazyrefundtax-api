...
exports.fetchClientTaxations= async (req, res) => {
    try {
      ...
      res.status(200).json(successResponse(200,true,"Data successfully fetched",{ totalData, limitedData} ));
    } catch (error) {
      console.error(error);
  ...
  exports.assignPreparer=async (req, res) => {
    try {
        ...
        if (!preparer) {
          return res.status(404).json(failedResponse(404,false,"Preparer not found." ));
        }
      
        // Check if client yearly taxation record exists
        const clientYearlyTaxation = await ClientYearlyTaxation.findById(clientYearlyTaxationId);
        if (!clientYearlyTaxation) {
          return res.status(404).json(failedResponse(404,false,"Client yearly taxation record not found."));
        }
      
        // Check if caller of client yearly taxation matches with callerId
        if (clientYearlyTaxation.caller!== callerId) {
          return res.status(403).json(failedResponse(403,false,"Caller does not have permission to assign preparers." ));
        }
        ...
        if (!updatedRecord) {
          return res.status(500).json(failedResponse(500,false,"Something went wrong."));
        }
        preparer.clientYearlyTaxations.push(clientYearlyTaxationId);
        await preparer.save();
      
        res.status(201).json(successResponse(201,true,"Preparer assigned successfully."));
      } catch (error) {
        console.error(error);
        res.status(500).json(failedResponse(500,false,"Internal Server Error."));
  ...
  exports.fetchClientTaxationById = async (req, res) => {
    try {
      ...
      if (!mongoose.Types.ObjectId.isValid(clientYearlyTaxationDetails)) {
        return res.status(400).json(failedResponse(400,false,"Caller does not have permission to access client yearly taxation."));
        
      }
      const taxData = await ClientYearlyTaxation.findById(callerId);
      if (!taxData) {
        return res.status(404).json(failedResponse(404,false,"This id doesn't exist."));
        
      }
      if (taxData.caller !== callerId) {
        return res.status(403).json(failedResponse(403,false,"Caller does not have permission to access client yearly taxation."));
      }
      ...
      res.status(200).json(successResponse(200,true,"Successfully data fetched",clientYearlyTaxationDetails[0]));
    } catch (error) {
      console.error(error);
      res.status(500).json(failedResponse(500,false,"Internal Server Error"));
    }
